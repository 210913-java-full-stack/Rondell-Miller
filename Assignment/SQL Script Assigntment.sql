...
###########################################################
DROP DATABASE IF EXISTS ronMillerDB;
CREATE DATABASE ronMillerDB;
...
-- SELECT SUM(a.balance) AS "Mr/Mrs.Smith Total"
-- FROM accounts a
...
-- SELECT a.address, c.name, a2.balance 
-- FROM address a 
	-- INNER JOIN customers c ON a.address_id = c.address_id 
	-- INNER JOIN accounts_customers ac ON c.customer_id = ac.customer_id 
	-- INNER JOIN accounts a2 ON ac.account_id = a2.account_id
-- WHERE a2.balance < 50;

# Get a list of all the customers who live in Texas.
-- SELECT c.name, a.state  FROM customers c 
